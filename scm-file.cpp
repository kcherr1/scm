...
static bool debug_indices = false;
//------------------------------------------------------------------------------
    ...
    zv(0), zc(0)
{
    ...
    if (!path.empty())
    {
        if (TIFF *T = TIFFOpen(path.c_str(), "r"))
        {
            ...
            scm_clog("gray", "Info from tiff '%s'\n--------------------", path.c_str());
            scm_clog("gray", "image width (w): %u", w);
            scm_clog("gray", "image length (h): %u", h);
            scm_clog("gray", "bits per sample (b): %u", b);
            scm_clog("gray", "samples per pixel (c): %u", c);

            // Preload all metadata.

            if (TIFFGetField(T, 0xFFB1, &n, &p))
            {
                scm_clog("gray", "got 0xffb1: %ull", n);
            ...
            {
                scm_clog("gray", "FAILED 0xffb1");
            }

            if (TIFFGetField(T, 0xFFB2, &n, &p))
            {
                scm_clog("gray", "got 0xffb2: %ull", n);
            ...
            {
                scm_clog("gray", "FAILED 0xffb2");
            }

            if (TIFFGetField(T, 0xFFB3, &n, &p))
            {
                scm_clog("gray", "got 0xffb3: %ull", n);
                if ((av = malloc(n * b / 8)))
                {
                    memcpy(av, p, n * b / 8);
                    ac = n;
            ...
            {
                scm_clog("gray", "FAILED 0xffb3");
            }

            if (TIFFGetField(T, 0xFFB4, &n, &p))
            {
                scm_clog("gray", "got 0xffb4: %ull", n);
                if ((zv = malloc(n * b / 8)))
                {
                    memcpy(zv, p, n * b / 8);
                    zc = n;
...
float* scm_file::get_page_sample4v(const double *v)
{
    float *result;
    if (xc)
    {
        ...
        if (sampler)
        {
            return sampler->get4v(v);
        }
        else
        {
            result = (float*)calloc(4, sizeof(float));
            for (int i = 0; i < 4; i++)
                result[i] = 1.f;
            return result;
        }
...
uint64 scm_file::find_page(long long a, double& y, double& x) const
{
    ...
    while ((j = toindex(scm_page_index(a, l, int(2 * n * y), int(2 * n * x)))) < oc)
        if (ov[j])
        {
            o = ov[j];
            l = l + 1;
...
static void set_text(const char *s, int x, int y, int w, int h, int c, int b, void *p)
{
    while (s[0])
    {
        for (int i = 0; i < 8; ++i)
...
void scm_page_text(const char *mesg, const char *name, long long i, int w, int h, int c, int b, void *p)
{
    char diag[256];
    ...
    char *logmsg = (char*)calloc(256 + strlen(mesg) + strlen(name) + 20, sizeof(char));
    sprintf(logmsg, "mesg:%s  file:%s  diag:%s", mesg, name, diag);
    scm_log(logmsg);
}

void scm_page_text_index_only(long long i, int w, int h, int c, int b, void *p)
{
    for (int j = 0; j < h; ++j)
    {
        set_pixel(1.f, 0, j, w, c, b, p);
    ...
    char diag[11]; // 10 digit max on ints, 1 extra for null
    snprintf(diag, 11, "%d", int(i));
    int dsz = strlen(diag);
    set_text(diag, w / 2 - dsz * 4, h / 3 - 4, w, h, c, b, p);
}
...
bool scm_load_page(const char *name, long long i, TIFF *T, uint64 o, int w, int h, int c, int b, void *p)
{
    if (T)
    {
        if (TIFFSetSubDirectory(T, o))
        {
            ...
            if (int(W) == w && int(H) == h && int(B) == b && int(C) == c)
            {
                ...
                if (debug_indices)
                    scm_page_text_index_only(i, W, H, C, B, p);
            }
            else
            {
                scm_page_text("LP: Bad page format", name, i, W, H, C, B, p);
            }
        }
        else
        {
            scm_log("LP: Page not found {w:%d, h:%d, c:%d, b:%d, p:%u, i:%lld, o:%ull}", w, h, c, b, *((uint8*)p), i, o);
            scm_page_text("LP: Page not found", name, i, w, h, c, b, p);
        }
    }
    else
    {
        scm_page_text("LP: File not found", name, i, w, h, c, b, p);
    }
